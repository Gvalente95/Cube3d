...
/*   Updated: 2025/03/24 08:29:30 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	show_fps(t_md *md, t_vec2 pos)
{
	...
	txt_data = get_v4(pos.x, pos.y, color, md->prm.txt_scale);
	rnd_abs_txt(md, txt_data, "fps:	%d", md->timer.prv_fps);
	if (!md->prm.debug_mode)
		return ;
	txt_data.b = -1;
	txt_data.g -= md->prm.txt_scale * 1.5;
...
void	show_update_information(t_md *md)
{
	...
	show_float(md, "FOV: ", md->prm.fov, get_v2(0, y++));
