...
/*   Updated: 2025/04/28 16:02:43 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*txt;
	const char	*label = md->txd.pickup_names[item_index];
	...
	{
		if (md->inv.held_index != item_index && md->inv.held_screen_img)
		{
			md->inv.items[md->inv.held_index]++;
			free_image_data(md, md->inv.held_screen_img);
			md->inv.held_screen_img = NULL;
		}
		inv->held_index = item_index;
