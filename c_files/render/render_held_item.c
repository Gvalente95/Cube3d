...
/*   Updated: 2025/04/28 16:05:38 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec2	get_pos(t_md *md, t_vec2 sz, float t)
{
	...
	if (!md->inv.held_used || !md->cam.pointed_ent)
		return (add_vec2(md->inv.pkbl_p, get_v2(mouse.delta.x, mouse.delta.y)));
...
{
	if (inv->held_index == Pokeball && sel && sel->type == nt_pokemon)
	{
		sel->caught = 1;
		capture_pokemon(md, inv, sel);
		add_log_to_queue(md, _GREEN, "%s was caught", sel->label);
	}
	else if (inv->held_index == Pokeball)
		add_alert(md, 2, NULL, "Not a Pokemon");
	if (inv->held_index == Keys && door)
	{
...
void	render_used(t_md *md, t_inventory *inv, double dur, int item_index)
{
	...
	sz = get_size(md, elapsed_time, dur, inv->held_sz.x);
	if (!cmp_vec2(sz, inv->held_screen_img->size))
	{
		free_image_data(md, inv->held_screen_img);
		inv->held_screen_img = \
			copy_image(md, md->txd.pickup_txtr[item_index][0], sz, -1);
	}
	pos = get_pos(md, sz, fminf(elapsed_time / dur, 1.0f));
	inv->pkbl_p = pos;
	draw_rotated(md, inv->held_screen_img, md->screen, \
		(t_vec3f){(float)pos.x, pos.y, inv->pkbl_rot_z});
	render_used_shadow(md, pos, sz);
	if (inv->held_used && (item_index == Keys || elapsed_time >= dur * 1.5))
...
void	render_held_item(t_md *md, t_inventory *inv, int hand_index)
{
	...
	{
		base = md->txd.pickup_txtr[hand_index][0];
