...
/*   Updated: 2025/03/26 16:26:18 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_player(t_md *md, t_ent *e)
{
	...
	decc_mv = get_v3f(e->mov.x * ACCSPD, e->mov.y * ACCSPD, e->mov.z * ACCSPD);
	if (e->pos.z + sign_mv.z > 0)
	{
		e->pos.z = 0;
		decc_mv.z = 0;
		sign_mv.z = 0;
	...
	e->mov = get_v3f(e->mov.x - decc_mv.x, e->mov.y - decc_mv.y, e->mov.z);
		...
		t_vec3f for_dir, t_vec3f rgt_dir)
{
	...
	if (md->key_clicked == SPACE_KEY)
		play_random_sound(md, AU_WALK_STEP, 5);
	return (mov);
}
...
int	update_player_mov(t_md *md)
{
	...
	if (md->plr.pos.z + md->prm.height < 0)
		md->plr.mov.z += GRAVITY;
	else if (md->plr.pos.z + md->prm.height > 1)
	{
