...
/*   Updated: 2025/04/03 12:33:47 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_plr_offsets(t_md *md)
{
	...
	md->input_offst.x += (int)(md->plr_wrd_mv.x);
...
t_vec3f	update_fly_cam(t_md *md, float spd)
{
	...
	if (md->key_prs[NUM_R_KEY])
		mov.z -= spd * .05;
	md->input_mov.x = md->key_prs[NUM_A_KEY] - md->key_prs[NUM_D_KEY];
	md->input_mov.y = md->key_prs[NUM_W_KEY] - md->key_prs[NUM_S_KEY];
	md->input_mov.z = md->key_prs[NUM_E_KEY] - md->key_prs[NUM_Q_KEY];
	return (mov);
}
