...
static void	init_cursor(t_md *md)
{
	...
	md->center = init_img_data(md, get_v2(CROSS_SCALE, CROSS_SCALE), "png/utils/center.png", -1);
	md->cursor = init_img_data(md, get_v2(30, 30), "png/utils/cursor/default.png", -1);
	md->curs_dtc = init_img_data(md, get_v2(30, 30), "png/utils/cursor/hand_open.png", -1);
	md->curs_grb = init_img_data(md, get_v2(30, 30), "png/utils/cursor/hand_closed.png", -1);
	mlx_hook(md->win, 5, 0, mouse_release_handler, md);
	mlx_hook(md->win, 6, 1L << 6, mouse_motion_handler, md);
}
