...
/*   Updated: 2025/04/23 10:46:36 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	pxl_draw(t_md *md, t_ray_draw_d *d, t_vec2 win_sz)
{
	...
	if (d->win_y < 0 || d->txd_crd.x < 0 || \
		d->win_y >= d->img->size.y || d->txd_crd.x >= d->img->size.x)
		return (0);
...
static void	draw_wall_strip(t_md *md, t_ray_draw_d *d, float step)
{
	...
	if (md->key_click == C_KEY && !d->ray->had_door && \
...
int	draw_wall_line(t_md *md, float dist, t_ent *hit, t_ray *ray)
{
	...
	update_door_sel(md, ray, ray->wall_hit, ray->wall_strip_pos);
	return (1);
}
