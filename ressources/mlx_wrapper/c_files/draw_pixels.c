...
/*   Updated: 2025/03/25 02:32:00 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	put_pixel_if_visible(
	t_draw_data *dd, int overlap, int blend, float opacty)
{
	t_image	*src_img;
	t_image	*dst;
	int		src_i;
...
void	draw_line(t_image *onto, t_vec2 start, t_vec2 end, t_vec2 color_d)
{
	...
	while (++i <= steps)
	{
		...
		if (color_d.y > 1)
			...
			draw_pixel(onto, start, color_d.x, 0.5);
